...
public:
    ...
    void upgrade_to_ssl(std::string&& cn_hostname, ssl_context_t& ssl_context)
    {
        std::lock_guard<std::mutex> lock(m_socket_lock);
        assert(!is_ssl());
        m_cn_hostname = std::move(cn_hostname);
        ssl_context.set_server_info(Botan::TLS::Server_Information(m_cn_hostname));
...
public:
    ...
    std::shared_ptr<asio_connection> obtain_connection(const http_request& req)
    {
        ...
        if (conn == nullptr)
        {
            ...
            {
                conn->upgrade_to_ssl(std::move(cn_host), this->client_config().get_ssl_context());
            }
        }
...
private:
    void upgrade_to_ssl()
    {
        ...
        m_connection->upgrade_to_ssl(calc_cn_host(client.base_uri(), m_request.headers()),
                                     client.client_config().get_ssl_context());
    }
